...
def load_video_conferencing(deltadata_path):
    archive_date = extract_archive_date(deltadata_path)
    print(f"Processing video conferencing for archive date: {archive_date}")

    filepath = os.path.join(deltadata_path, "curr_video_conferencing.csv")

    if not os.path.exists(filepath):
        print("  curr_video_conferencing.csv not found - skipping.")
        return
    ...
    with open(filepath, newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            record = RawVideoConferencing(
                unit_id      = parse_int(row.get("unit_id")),
                archive_date = archive_date,
                dtime_local  = parse_timestamp(row.get("dtime")),
                dtime_utc    = None,
    ...
    if len(records) == 0:
        print("  curr_video_conferencing.csv is empty - skipping.")
        return
    ...
    try:
        for record in records:
            session.add(record)
        session.commit()
        print(f"  curr_video_conferencing: {len(records)} rows loaded.")
    except Exception as e:
        session.rollback()
        print(f"  Error loading video conferencing: {e}")
    finally:
        session.close()


if __name__ == "__main__":
    load_video_conferencing(
